...
def initiate_session():
    """set up session variables that persist per browser for multiple requests."""
    session['sid'] = 'Session ID: {0}\n'.format(str(uuid4()))
    session['count'] = 0
    session['aggregate_texts'] = list()
    session['prev_outputs'] = list()
    session['leaflet'] = False
    pass
...
def ask(clf=naive_bayes_classifier, engine=Engine):
    ...
    session['aggregate_texts'].append(question)

    output = clf(query=' '.join(session['aggregate_texts']), engine=engine)
